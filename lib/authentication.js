...
exports.decryptTokenWithKeys = function (token, keys) {
  ...
  for (i = 0; i < keys.length; ++i) {
    ...
    try {
      ...
      console.log("successfully decrypted token with keys");
      return decryptedToken;
    } catch (e) {
      console.log("error decrypting token with keys")
      lastError = e;
    }
  }
...
exports.verifyAudience = function (token, audiences) {
  ...
  const audienceIntersection = !!intersection.length

  if (!audienceIntersection) {
    console.log(`verify audiences failed: possibly missing ${names}`)
  }

  return audienceIntersection;
};
