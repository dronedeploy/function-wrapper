...
exports.getPublicKeys = function (config) {
  let env = config.env || global.NODE_ENV || "prod";  // default to prod
  let envUrlPart = env === "prod" ? "" : "_" + env;
  const endpoint = `https://api${envUrlPart}.dronedeploy.com/api/v1/jwt_public_keys`;
